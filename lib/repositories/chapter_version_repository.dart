...
class ChapterVersionRepository {
  ...
  Future<List<Paragraph>?> fetchContentModeration(
    ...
    final url = Uri.parse(
        "$chapterVersionEndpoint/$chapterVersionId/content-moderations");
    Map<String, String> header = {
      "Content-type": "application/json",
      "Accept": "application/json"
    };
    ...
    if (response.statusCode == 200) {
      try {
        final List<dynamic> result = jsonDecode(responseBody)['data'];
        return result.map((i) => Paragraph.fromJson(i)).toList();
      } catch (error) {
        print(error);
        throw (error);
      }
